...
async function main() {
  ...
  const members = await Promise.all([
    prisma.member.create({ data: { name: "Ricardo Almeida", email: "ricardo@empresa.com", role: "Gestor" } }),
    prisma.member.create({ data: { name: "Ana Costa", email: "ana@empresa.com", role: "Operações" } }),
    prisma.member.create({ data: { name: "João Santos", email: "joao@empresa.com", role: "Atendimento" } }),
    prisma.member.create({ data: { name: "Maria Silva", email: "maria@empresa.com", role: "Comercial" } }),
    prisma.member.create({ data: { name: "Pedro Lima", email: "pedro@empresa.com", role: "Financeiro" } }),
    prisma.member.create({ data: { name: "Juliana Rocha", email: "juliana@empresa.com", role: "Marketing" } }),
    prisma.member.create({ data: { name: "Carlos Souza", email: "carlos@empresa.com", role: "Suporte" } }),
    prisma.member.create({ data: { name: "Fernanda Alves", email: "fernanda@empresa.com", role: "Produto" } }),
    prisma.member.create({ data: { name: "Lucas Oliveira", email: "lucas@empresa.com", role: "Analista" } }),
    prisma.member.create({ data: { name: "Rafael Gomes", email: "rafael@empresa.com", role: "Design" } }),
  ]);
  ...
  const portalCliente = await prisma.project.create({
    data: {
      ...
      description: "Centralização de solicitações e acompanhamento de clientes.",
      startDate: addDays(-10),
      dueDate: addDays(12),
      status: ProjectStatus.ACTIVE,
      priority: Priority.HIGH,
      members: {
        connect: [ana, joao, maria, carlos].map((member) => ({ id: member.id })),
      },
    },
  });
  ...
  const sistemaFinanceiro = await prisma.project.create({
    data: {
      ...
      description: "Organização de contas, pagamentos e relatórios financeiros.",
      startDate: addDays(-14),
      dueDate: addDays(2),
      status: ProjectStatus.AT_RISK,
      priority: Priority.URGENT,
      members: {
        connect: [ricardo, pedro, ana, joao].map((member) => ({ id: member.id })),
      },
    },
  });
  ...
  const siteInstitucional = await prisma.project.create({
    data: {
      ...
      members: {
        connect: [juliana, rafael, fernanda].map((member) => ({ id: member.id })),
      },
    },
  });
